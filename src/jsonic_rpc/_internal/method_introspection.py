...
def _parameter_is_dependency(parameter: Parameter):
    ...
    return get_origin(annotation) is Annotated or any([isinstance(arg, Depends) for arg in get_args(annotation)])


def method_non_depends_args(method_signature: Signature) -> Sequence[Parameter]:
    return [parameter for parameter in method_signature.parameters.values() if not _parameter_is_dependency(parameter)]


def method_depends_args(method_signature: Signature) -> Sequence[Parameter]:
    return [parameter for parameter in method_signature.parameters.values() if _parameter_is_dependency(parameter)]
...
def get_method_name(method: Method, name: str | None) -> str:
    ...
    if not method_name:
        raise ValueError("Cannot access method __name__ attribute, so you have to explicitly specify method name")
    return method_name
